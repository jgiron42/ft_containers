...
namespace nstest_map {
	...
	void test_at(C &m1, C &m2) {
		...
		for (int i = 0; i < 5; i++) {
			try {
				...
				std::cout << m1.at(get_value<typename C::key_type>());
				std::cout << m2.at(get_value<typename C::key_type>());
			}
	...
	template<class C, template<class, class> class P>
	void test_equal_range(C &m1, C &m2) {
	...
	template<class C, template<class, class> class P>
	void test_upper_bound(C &m1, C &m2) {
	...
	template<class C, template<class, class> class P>
	void test_lower_bound(C &m1, C &m2) {
